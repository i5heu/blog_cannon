...
func PageHandler(w http.ResponseWriter, r *http.Request) {
	...
	if TMPCACHEWRITE == false {
		list = lista{template.HTML(conf.BlogName), time.Since(start), TMPCACHE[slug], true, conf.CoinHiveToken, conf.TrackTrackGo}
	} else if TMPCACHECACHEWRITE == false {
		list = lista{template.HTML(conf.BlogName), time.Since(start), TMPCACHECACHE[slug], true, conf.CoinHiveToken, conf.TrackTrackGo}
	} else {
		list = lista{template.HTML(conf.BlogName), time.Since(start), template.HTML("<b> CACHE ERROR <br> Please reload this page</b>"), true, conf.CoinHiveToken, conf.TrackTrackGo}
	}
